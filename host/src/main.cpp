...
//    gettimeofday(&tstart, NULL);//测量代码时间开始
    ...
    std::string binary_file = getBoardBinaryFile("Erosion_Operation", device);
...
//    cout << "Probe Used Time = " << timeuse << "uS" << endl;
    ...
    kernel = clCreateKernel(program, "erosion", &status);
...
void Erosion(Mat &img, const unsigned int pixels_num, unsigned int rows, unsigned int cols)
{
    size_t SobelSize = 1;
    cl_int status;
...
//    gettimeofday(&tend, NULL);//结束测量
    ...
    Erosion(in_img, IN_IMG_SIZE, in_img_rows, in_img_cols);
    ...
    imwrite("test_cv_erosion.jpg", dst);
	gettimeofday(&tend, NULL);//结束测量
	timeuse = (unsigned int) (1000000*(tend.tv_sec-tstart.tv_sec)+(tend.tv_usec-tstart.tv_usec)); //换算为微秒
